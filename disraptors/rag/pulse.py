...
class Pulse:
    ...
    def rag_qa(
        ...
        try:
            chunks = asyncio.run(self.retriever.aget_relevant_documents(retrieve_query, filter_=filter_, **kwargs))
        except Exception as e:
    ...
    def is_greetings(self, query: str):
        prompt = PromptTemplate.from_template("Is this a greeting?If yes, say YES, otherwise, say NO.\n{query}")
        check_chain = prompt | self.llm
        res = check_chain.invoke(query)
        return True if res.content == "YES" else False
    
    def chat(self, query: str, time_delta: dict = {"years": 2}, **kwargs):
        if self.is_greetings(query):
            return self.llm.invoke(query).content
        else:
            return self.rag_qa(query=query, retrieve_query=query, time_delta=time_delta, **kwargs)
