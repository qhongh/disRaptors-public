...
st.header(":chart_with_upwards_trend: Trend - Synopsis on top competitor funds")
st.caption("🚀 Powered by Claude, built on reddit, twitter and you-tube interactions.")

#st.selectbox(label = "Select the trending non-Vanguard fund.", options = ("ticker 1", "ticker 2"))
...
def load_data(df_type):
    ...
    if df_type == "trend":
        return pd.read_excel(data_path / 'trend_output.xlsx')
...
selected_fund = st.selectbox(label = "_Select the Competitor fund._", options = df_trend["ticker"])
...
def parse_md(text) -> List:
    chunks = re.split(r'[0-9]\.', text)
    return chunks

def parse_chunk(chunk) -> List:
    subchunk = re.split(r':', chunk, maxsplit=1)
    return subchunk



trends_tab, topics_tab = st.tabs(['Trending Fund', 'Emerging Topics'])

with trends_tab:
    with st.expander(":blue[Summarized social media sentiment.]"):
        with st.spinner('Wait for it...'):
            time.sleep(0.5)
        summary = df_trend.query(f"ticker == '{selected_fund}'")["synopsis"].iloc[0]
    ...
    with st.expander(":blue[Cash flow and similarity insights.]"):
        with st.spinner('Wait for it...'):
            time.sleep(0.5)
        fig = find_similar_tickers_and_plot_cashflows(ticker=selected_fund, history_window_length=50, top_k = 3, include_one_interal_vanguard=True)
        st.plotly_chart(fig, use_container_width=True)


with topics_tab:
    topics_md = (app_folder / 'output_datasets/trend.md').read_text()
    chunked = parse_md(topics_md)
    title_body = [parse_chunk(c) for c in chunked]

    for chunk in title_body:
        if len(chunk) >= 2: # Remove/Skip first empty string element from list
            with st.expander(chunk[0]):
                st.markdown(chunk[1])

    #st.markdown(topics_md, unsafe_allow_html=True)
